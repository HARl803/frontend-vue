<template>
  <div class="container mt-4 mb-4 flex-grow-1">
    <div class="row mb-5">
      ...
      <div class="col-12 col-lg-8">
          ...
          <div class="card-body">
            <div class="d-flex justify-content-between align-items-center category-header">
              <h5 class="font-weight-bold">
                ...
                <span>총부품수: {{ Object.keys(filteredParts).length }}개<br /></span>
              </h5>
              <button class="btn btn-outline-secondary btn-sm toggle-button" @click="toggleAllCategories">
        ...
        <!-- 필터 표시 영역 -->
        <div>
          <div v-for="(filterOptions, filterName) in visibleFilters" :key="filterName" class="mb-3">
            <div class="row mb-1 align-items-start">
              <div class="col-2">
                <div class="filter-title font-weight-bold">{{ filtersStore.getters[currentCategory][filterName] }}</div>
              </div>
              <div class="col-9">
                <div class="d-flex flex-wrap align-items-start">
                  <div v-for="(option, index) in filterOptions" :key="option" class="option-box mb-1" v-show="index < 5 || showFilterOptions[filterName]">
                    <div class="form-check small">
                      <input class="form-check-input" type="checkbox" :value="option" :id="option" v-model="selectedFilters" />
                      <label class="form-check-label text-truncate" :for="option" title="{{ option }}">{{ option }}</label>
                    </div>
                  </div>
                </div>
              </div>
              <div class="col-1 text-center">
                <button class="btn btn-link btn-sm" @click="toggleFilterVisibility(filterName)">
                  <i :class="showFilterOptions[filterName] ? 'bi bi-chevron-up' : 'bi bi-chevron-down'"></i>
                </button>
              </div>
            </div>
          </div>
      ...
      <div class="col-12 col-lg-4">
        <div class="d-flex justify-content-between align-items-center mb-2 comparison-header">
          <button class="btn btn-success w-50 dropdown-toggle" @click="toggleDropdown">내 PC 불러오기</button>
          <button class="btn btn-primary w-50 ms-2" @click="recommendAI">AI 견적 추천</button>
        </div>
        <div class="pc-management">
          <div v-if="showDropdown" class="dropdown-menu w-100 p-2" style="display: block">
            <div v-for="pc in pcStore.pcList" :key="pc.id" class="card mb-2">
              <div class="card-header d-flex justify-content-between align-items-center">
                <div>
                  <input v-if="pc.editing" v-model="pc.computerName" @blur="savePCName(pc)" @keyup.enter="savePCName(pc)" class="form-control form-control-sm me-2" />
                  <span v-else @dblclick="editPCName(pc)">{{ pc.computerName }}</span>
                  <i class="bi bi-pencil ms-2" @click="editPCName(pc)"></i>
                </div>
                <div>
                  <button class="btn btn-primary btn-sm" @click="loadPC(pc)">불러오기</button>
                  <button class="btn btn-danger btn-sm ms-2" @click="removePC(pc.id)">삭제</button>
                </div>
              </div>
            </div>
            <div class="text-center mt-3">
              <button class="btn btn-success btn-sm" @click="addPC()">추가하기</button>
            </div>
          </div>
          <div class="list-group mt-3">
            <div v-for="(categoryLabel, categoryKey) in categoryLabels" :key="categoryKey" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center" @click="setCategory(categoryKey)">
              <div class="d-flex align-items-center">
                <span class="me-3">{{ categoryLabel }}</span>
                <span v-if="pcStore.pcList[0][categoryKey]" class="badge bg-success">
                  {{ partsData[categoryKey][`${pcStore.pcList[0][categoryKey]}`].manufacturer }} - 
                  <span v-if="categoryKey == 'cpu'">{{ partsData[categoryKey][`${pcStore.pcList[0][categoryKey]}`].cpuType }}</span>
                  <span v-if="categoryKey == 'memory'">{{ partsData[categoryKey][`${pcStore.pcList[0][categoryKey]}`].deviceType }}</span>
                  <span v-if="categoryKey == 'gpu'">{{ partsData[categoryKey][`${pcStore.pcList[0][categoryKey]}`].chipsetManufacturer }}</span>
                  <span v-if="categoryKey == 'ssd'">{{ partsData[categoryKey][`${pcStore.pcList[0][categoryKey]}`].formFactor }}</span>
                  <span v-if="['coolerTuning', 'motherboard', 'hdd', 'case', 'power'].includes(categoryKey)">
                    {{ partsData[categoryKey][`${pcStore.pcList[0][categoryKey]}`].productType }}
                  </span>
                </span>
                <span v-else class="badge bg-secondary">선택 안됨</span>
              </div>
              <button v-if="pcStore.pcList[0][categoryKey]" class="btn btn-danger btn-sm" @click.stop="removeFromCart(categoryKey)">삭제</button>
            </div>
          </div>
        </div>
  ...
  <!-- 선택된 필터 저장 영역 -->
  <div class="cart mt-3 mb-3 bg-dark text-white p-2 rounded d-flex align-items-center justify-content-between">
    <div class="d-flex align-items-center flex-grow-1">
      <button class="btn btn-outline-danger btn-sm me-2 reset-button" @click="clearFilters">검색 초기화</button>
      <div v-if="selectedFilters.length" class="d-flex flex-wrap align-items-center">
        <div v-for="filter in selectedFilters" :key="filter" class="cart-item bg-secondary text-white p-1 m-1 rounded d-flex align-items-center small">
          {{ filter }}
          <button class="btn btn-link btn-sm text-danger ms-1 p-0" @click="removeFilter(filter)">X</button>
        </div>
      </div>
    </div>
    <!-- 검색어 입력 영역 -->
    <div class="d-flex align-items-center">
      <div class="input-group">
        <input type="text" class="form-control search-input" placeholder="부품명을 검색하세요 ..." v-model="searchQuery" />
  ...
  <div class="comparison-area mb-3 p-3 bg-dark text-white rounded">
    ...
    <!-- 비교할 부품 영역 -->
    <div v-if="isComparisonVisible" class="row h-100 d-flex align-items-center justify-content-center">
      <!-- 비교할 부품이 없는 경우 -->
      <div v-if="comparisonParts.length === 0" class="col-12 d-flex justify-content-center text-center">
      ...
      <!-- 비교할 부품이 하나인 경우 -->
      <div v-else-if="comparisonParts.length === 1" class="col-12 d-flex justify-content-center text-center">
        <div class="mx-3">
          <div class="part-container">
            <h5>{{ comparisonParts[0].name }}</h5>
            <p>{{ comparisonParts[0].description }}</p>
            <button class="btn btn-danger btn-sm ms-2" @click="removeFromComparison(0)">제거</button>
          </div>
        </div>
        <div class="mx-3 text-center">
          <img src="/src/views/partsSearch/monitor.png" class="default-image" />
            ...
            <thead>
              <tr>
                ...
                <th v-for="(part, index) in comparisonParts" :key="index">
                  {{ part.name }}
                  <button class="btn btn-danger btn-sm ms-2" @click="removeFromComparison(index)">제거</button>
                </th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="attribute in attributes" :key="attribute">
                ...
                <td v-for="(part, index) in comparisonParts" :key="index">
                  <!-- 속성값이 객체인 경우, 콤마로 구분하여 출력 -->
                  <span v-if="typeof part[attribute] === 'object'">
                    {{ Object.values(part[attribute]).join(', ') }}
                  </span>
                  <!-- 속성값이 .jpg URL인 경우 -->
                  <span v-else-if="part[attribute].endsWith('.jpg')">
                    <img :src="part[attribute]" alt="부품 이미지" style="max-width: 100px; max-height: 100px;" />
                  </span>
                  <!-- 속성값이 객체도 아니고 .jpg URL도 아닌 경우, 그대로 출력 -->
                  <span v-else>
                    {{ part[attribute] }}
                  </span>
                </td>
  ...
  <div class="container">
    <div class="row">
      <div v-for="(part, partId) in filteredParts" :key="partId" class="col-12 col-md-6 col-lg-3 mb-4">
        <div class="card position-relative h-100" @click="viewPartDetail(partId)">
          ...
          <div class="card-body d-flex flex-column">
            ...
            <div class="button-group mb-3">
              <div v-for="element in part.description.split(',')" :key="element">
                <button class="btn btn-outline-primary btn-sm custom-button">{{ element }}</button>
              </div>
            </div>
            <div class="d-flex justify-content-between align-items-center mt-auto">
              <div>
                <button class="btn btn-sm btn-dark me-2" @click.stop="addToComparison(part)">부품 비교에 추가</button>
                <button class="btn btn-sm btn-dark" @click.stop="addToCart(partId)">내 PC에 추가</button>
              </div>
            </div>
          </div>
        </div>
